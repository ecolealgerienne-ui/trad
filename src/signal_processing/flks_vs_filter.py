...
def kalman_flks(z: np.ndarray, lag: int = 2):
    ...
    At time t, produces a smoothed estimate of state at time t-N,
    using observations z[0..t]. This is CAUSAL (no future data beyond t).

    Implementation: At each step t, run forward filter up to t,
    then run RTS backward for N steps (from t back to t-N).

    Optimized version: use the recursive FLKS formulas.
    For each t >= lag:
        x_smooth[t-lag|t] = x_filt[t-lag|t-lag] + sum of corrections

    We use the efficient formulation from Anderson & Moore (1979):
    Maintain a buffer of N+1 filter states and apply backward smoothing.

    Returns:
        x_flks: (T, 2) smoothed states (x_flks[t] = estimate of x[t] using z[0..t+lag])
                For t < lag, falls back to filter estimate.
    """
    x_filt, P_filt, x_pred, P_pred = kalman_filter_forward(z)
    T = len(z)
    ...
    # For each time t, we want the smoothed estimate of x[t] given z[0..t+lag]
    # This means at observation index (t+lag), we smooth back to index t
    for t in range(T):
        end = min(t + lag, T - 1)
        if end <= t:
            # Not enough future data, keep filter estimate
            continue

        # Run local backward RTS from end back to t
        # We reuse the already-computed forward filter states
